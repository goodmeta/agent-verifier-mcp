...
const usd = (cents: number | string): string => `$${(Number(cents) / 100).toFixed(2)}`
const remainingText = (mandate: { remainingBudget?: string } | undefined): string =>
  mandate?.remainingBudget !== undefined ? usd(mandate.remainingBudget) : 'uncapped'
...
server.tool(
  ...
  async ({ hold_id }) => {
    ...
    if (Number(r.refundedAmount) > 0) lines.push(`  Refunded: ${usd(r.refundedAmount)}`)
    return text(lines.join('\n'))
  }
)
